...
export async function getPlannerOptionsForTenant(tenant: TenantKey): Promise<PlannerOptionsResponse> {
  ...
  const [cityRows, destinationRows] = await Promise.all([
    ...
    }).from(plannerDestinations).where(and(eq(plannerDestinations.tenantId, tenant.id), eq(plannerDestinations.status, "published"))).orderBy(asc(plannerDestinations.routeOrder), asc(plannerDestinations.displayOrder), asc(plannerDestinations.id)),
  ]);
