...
import { inquiries, plannerDestinations, tenantContactChannels, tenantHeroSlides, tenantSiteProfiles, tenants } from "../../db/schema";
...
export async function getAdminDashboard(tenantId: string): Promise<AdminDashboardData> {
  ...
  const [tenantRows, profileRows, contactRows, heroCountRows, destinationCountRows, inquiryCountRows, newInquiryCountRows] = await Promise.all([
