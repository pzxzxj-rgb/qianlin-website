  ...
  return (
    <section id="home" className="hero-section" onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
      <div className="hero-slides">
        {slides.length === 0 ? <p className="hero-empty-state" role="status">{t.hero.empty}</p> : slides.map((slide, index) => <div className={`hero-slide${visibleIndex === index ? " hero-slide-active" : ""}`} key={slide.id} style={{ "--hero-desktop-position": slide.desktopPosition, "--hero-mobile-position": slide.mobilePosition } as CSSProperties}><img src={slide.src} alt={visibleIndex === index ? slide.alt[language] : ""} aria-hidden={visibleIndex === index ? undefined : true} loading={index === 0 ? "eager" : "lazy"} fetchPriority={index === 0 ? "high" : "low"} /></div>)}
      </div>
      ...
      {slides.length > 0 ? <div className="hero-carousel-controls" role="group" aria-label={language === "zh" ? "首页图片轮播控制" : "Hero image carousel controls"} onFocus={() => setIsControlFocused(true)} onBlur={(event) => { if (!event.relatedTarget || !event.currentTarget.contains(event.relatedTarget as Node)) setIsControlFocused(false); }}>
        ...
        <div className="hero-carousel-dots">{slides.map((slide, index) => <button type="button" className={`hero-carousel-dot${visibleIndex === index ? " hero-carousel-dot-active" : ""}`} key={slide.id} onClick={() => goToSlide(index)} aria-label={language === "zh" ? `${t.hero.slide}${index + 1}张首页图片` : `${t.hero.slide} ${index + 1}`} aria-current={visibleIndex === index ? "true" : undefined} />)}</div>
