...
export function AdminDashboard({ data }: { data: AdminDashboardData }) {
  ...
  return <main className="admin-page">
    <header className="admin-topbar"><Link className="admin-brand" href="/"><span className="brand-mark">{data.profile.logoMark || "Q"}</span><span><strong>{data.profile.companyNameEn}</strong><small>管理后台</small></span></Link><div className="admin-topbar-actions"><Link className="admin-topbar-link" href="/admin/inquiries">咨询管理</Link><span className="admin-tenant-chip">{data.tenant.slug}</span><AdminLogoutButton /></div></header>
    <div className="admin-shell">
      ...
      <div className="admin-grid">
        ...
        <section className="admin-card admin-card-wide admin-inquiry-entry-card"><div className="admin-card-heading"><div><span className="eyebrow">INQUIRIES</span><h2>咨询管理</h2></div><Link className="button button-light admin-edit-profile-link" href="/admin/inquiries">查看咨询列表</Link></div><p>列表默认只显示脱敏联系方式，完整手机号、微信、邮箱和留言仅在详情页提供。</p></section>
