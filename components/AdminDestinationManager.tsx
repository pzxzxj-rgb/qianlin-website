...
type TextDestinationField = Exclude<keyof AdminDestinationInput, "cityCode" | "cardSize" | "routeOrder" | "recommendedVisitHours" | "majorAttraction" | "availableForPlanning" | "showOnHomepage" | "displayOrder" | "status">;
...
function isAdminDestinationValues(value: unknown): value is AdminDestinationValues {
  ...
  const isBoundedInteger = (candidate: unknown, min: number, max: number) => typeof candidate === "number" && Number.isInteger(candidate) && candidate >= min && candidate <= max;
  return typeof record.id === "string" && typeof record.slug === "string" && typeof record.cityCode === "string" && typeof record.cityNameZh === "string" && typeof record.cityNameEn === "string" && typeof record.nameZh === "string" && typeof record.nameEn === "string" && typeof record.descriptionZh === "string" && typeof record.descriptionEn === "string" && (record.cardSize === "small" || record.cardSize === "large") && typeof record.regionZh === "string" && typeof record.regionEn === "string" && isBoundedInteger(record.routeOrder, 0, 1000) && (record.recommendedVisitHours === null || isBoundedInteger(record.recommendedVisitHours, 1, 48)) && typeof record.majorAttraction === "boolean" && typeof record.availableForPlanning === "boolean" && typeof record.showOnHomepage === "boolean" && typeof record.hasHomepageImage === "boolean" && isBoundedInteger(record.displayOrder, 0, 1000) && (record.status === "draft" || record.status === "published" || record.status === "archived");
}
...
export function AdminDestinationManager({ initialValues, cityOptions }: { initialValues: AdminDestinationValues[]; cityOptions: AdminCityOption[] }) {
  ...
  function renderForm(values: AdminDestinationInput, formKey: string, isNew: boolean, destinationId?: string) {
    ...
    const hasHomepageImage = !isNew && destinationId ? destinations.find((destination) => destination.id === destinationId)?.hasHomepageImage === true : false;
    ...
    return <form className="admin-destination-form" aria-busy={pendingKey === formKey} onSubmit={(event) => { if (isNew) void handleCreate(event); else if (destinationId) void handleUpdate(event, destinationId); }}>
      ...
      <div className="admin-destination-field admin-destination-checkboxes"><label>目的地属性</label><label className="admin-checkbox-label"><input type="checkbox" checked={values.majorAttraction} disabled={pending} onChange={(event) => updateField(formKey, "majorAttraction", event.target.checked)} />主要景点</label><p className="admin-destination-help">当前仅作为目的地标签保存，不影响自动排序。</p><label className="admin-checkbox-label"><input type="checkbox" checked={values.availableForPlanning} disabled={pending} onChange={(event) => updateField(formKey, "availableForPlanning", event.target.checked)} />参与行程规划</label><label className="admin-checkbox-label"><input type="checkbox" checked={values.showOnHomepage && hasHomepageImage} disabled={pending || !hasHomepageImage} onChange={(event) => updateField(formKey, "showOnHomepage", event.target.checked)} />首页展示</label>{!hasHomepageImage ? <p className="admin-destination-help">当前目的地没有首页图片，只能用于行程规划。</p> : null}{errors.majorAttraction || errors.availableForPlanning || errors.showOnHomepage ? <p className="admin-field-error" role="alert">请检查目的地属性设置。</p> : null}</div>
