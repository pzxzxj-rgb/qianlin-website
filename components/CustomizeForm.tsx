...
export function CustomizeForm({ open, initialTourName = "", initialPlaces = "", initialMessage = "", onOpen, onClose, tenantSlug, siteConfig }: CustomizeFormProps) {
  ...
  return <>
    <section id="customize" className="section section-customize">
      <div className="container customize-layout">
        <div className="customize-copy">
          <span className="eyebrow eyebrow-light">{t.customize.eyebrow}</span>
          <h2>{t.customize.title}<br /><em>{t.customize.accent}</em></h2>
          <p>{t.customize.description}</p>
          <div className="customize-details">{t.customize.details.map((item) => <span key={item}>{item}</span>)}</div>
    ...
    {open ? <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) handleClose(); }}>
      <div ref={modalRef} className="customize-modal" role="dialog" aria-modal="true" aria-labelledby="customize-modal-title" aria-describedby="customize-modal-description">
        <div className="modal-header">
          <div><span className="eyebrow">{t.customize.modalEyebrow}</span><h2 id="customize-modal-title">{t.customize.modalTitle}</h2><p id="customize-modal-description" className="modal-description">{t.customize.modalDescription}</p></div>
          <button type="button" className="modal-close" onClick={handleClose} aria-label={t.customize.close}>×</button>
        </div>
