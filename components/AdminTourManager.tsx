...
const TEXT_FIELDS: Array<{ field: TextTourField; label: string; maxLength: number; multiline?: boolean; wide?: boolean }> = [
  ...
  { field: "priceTextZh", label: "中文参考价格（可选）", maxLength: 60 },
  { field: "priceTextEn", label: "英文参考价格（可选）", maxLength: 100 },
...
export function AdminTourManager({ initialValues, tenantSlug }: { initialValues: AdminTourValues[]; tenantSlug?: string }) {
  ...
  function renderForm(values: AdminTourInput, formKey: string, isNew: boolean, tourId?: string) {
    ...
    return <form className="admin-tour-form" aria-busy={pendingKey === formKey} onSubmit={(event) => { if (isNew) void handleCreate(event); else if (tourId) void handleUpdate(event, tourId); }}>
      ...
      <div className="admin-profile-actions"><button className="button button-dark" type="submit" disabled={pending || !isFormDirty}>{pendingKey === formKey ? "正在保存参考方案……" : isNew ? "新增参考方案" : "保存参考方案"}</button><button className="button button-light" type="button" disabled={pending} onClick={() => isNew ? handleNewCancel() : tourId ? handleEditCancel(tourId) : undefined}>{isNew ? "清空新增表单" : "取消修改"}</button></div>
    </form>;
  }

  return <main className="admin-page">
    <header className="admin-topbar"><Link className="admin-brand" href={adminPagePath(tenantSlug, "")} onClick={handleReturn}><span className="brand-mark">Q</span><span><strong>QIANLIN TRAVEL</strong><small>旅行灵感 / 参考方案管理</small></span></Link><div className="admin-topbar-actions"><Link className="admin-profile-back-link" href={adminPagePath(tenantSlug, "")} onClick={handleReturn}>返回后台</Link><AdminLogoutButton isDirty={isDirty} disabled={pending} /></div></header>
    <div className="admin-shell admin-tours-shell">
      <div className="admin-heading"><div><span className="eyebrow">QIANLIN TRAVEL · TOURS</span><h1>旅行灵感 / 参考方案管理</h1><p>这里用于管理官网展示的旅行灵感和参考方案，不代表固定团期或实时可售产品。没有稳定报价时，参考价格可以留空。</p></div></div>
      <p className="admin-tour-status" role="status" aria-live="polite">{pending ? "正在保存参考方案……" : "参考方案已加载"}{sessionExpired ? <Link href="/admin/login">重新登录</Link> : null}</p>
      ...
      <section className="admin-card admin-tours-card"><div className="admin-card-heading"><div><span className="eyebrow">NEW TOUR</span><h2>新增参考方案</h2></div><span className="admin-image-count">服务端生成线路 ID</span></div>{renderForm(newValues, "new", true)}</section>
      <section className="admin-card admin-tours-card"><div className="admin-card-heading"><div><span className="eyebrow">TOUR LIST</span><h2>已有参考方案</h2></div><span className="admin-image-count">{tours.length} 条记录</span></div>{tours.length === 0 ? <p className="admin-tour-empty">当前还没有参考方案。可以在获得已经确认、适合公开展示的旅行资料后从上方新增；如果目前没有资料，保持为空即可。</p> : <div className="admin-tour-list">{tours.map((tour, index) => <article className="admin-tour-card" key={tour.id}><div className="admin-tour-card-heading"><div><span className="eyebrow">TOUR {String(index + 1).padStart(2, "0")}</span><h3>{tour.titleZh}</h3><p>{tour.titleEn} · {tour.slug}</p></div><span className={`admin-tour-status admin-tour-status-${tour.status}`}>{STATUS_LABELS[tour.status]}</span></div>{renderForm(drafts[tour.id], tour.id, false, tour.id)}</article>)}</div>}</section>
    </div>
  </main>;
}
