...
export function Footer({ showTours, siteConfig }: FooterProps) {
  ...
  const copy = isDemo ? "Demo tenant page" : t.footer.copy;
  const copyTwo = isDemo ? "A preview of future tenant capabilities." : t.footer.copyTwo;
  const copyright = isDemo ? "Demo site" : t.footer.copyright;
  const companyLabel = isDemo ? "Site" : t.footer.company;
