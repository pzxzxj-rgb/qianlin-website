...
type ToursProps = { tours: readonly Tour[]; onBook: (tourName: string) => void; region?: { zh: string; en: string } };

export function Tours({ tours, onBook, region }: ToursProps) {
          ...
          {tours.map((tour, index) => {
            ...
            return <article className="tour-card" key={tour.id}>
              ...
              <div className="tour-card-body">
                {tour.duration ? <div className="tour-meta"><span>{tour.duration[language]}</span><span className="meta-dot" /><span>{region?.[language] || t.tours.location}</span></div> : null}
                <h3>{title}</h3>
