...
async function main() {
  resetRateLimitForTests();
  (process.env as Record<string, string | undefined>).APP_ENV = "test";
  ...
  assert.equal(getRequestAddress(new Request("http://localhost", { headers: { "X-Forwarded-For": "198.51.100.11, 198.51.100.12" } })), "local:198.51.100.11");
