...
async function request(baseUrl, pathname, init) { const response = await fetch(`${baseUrl}${pathname}`, { ...init, headers: { connection: "close", ...(init?.headers ?? {}) } }); const text = await response.text(); let body; try { body = JSON.parse(text); } catch { body = text; } return { response, body }; }
...
async function runScenario({ failure }) {
  ...
  try {
    ...
    for (let attempt = 0; attempt < 240; attempt += 1) {
      ...
      try { const ready = await fetch(`${baseUrl}/api/t/qianlin-travel/site-config`); if (ready.status === 200 && ready.headers.get("x-qianlin-readiness") === readinessToken) break; } catch {}
      await new Promise((resolve) => setTimeout(resolve, 250));
      if (attempt === 239) throw new Error(`sync test server did not become ready: ${output.join("").slice(-3000)}`);
    }
