...
try {
  ...
  const toursPage = await request("/admin/t/qianlin-travel/tours", { headers: { cookie: sessionCookie } });
  assert.equal(toursPage.response.status, 200);
  assert.match(toursPage.response.headers.get("cache-control") ?? "", /no-store/i);
  assert.match(String(toursPage.body), /<title>后台管理 \| 黔林旅行社<\/title>/);
  ...
  assert.match(String(toursPage.body), /旅行灵感 \/ 参考方案管理/);
  assert.match(String(toursPage.body), /新增参考方案/);
  assert.match(String(toursPage.body), /已有参考方案/);
  assert.match(String(toursPage.body), /参考方案已加载/);
  assert.doesNotMatch(String(toursPage.body), /新增虚构线路/);
