...
- 首页包含 Hero 主题图片轮播、线路、目的地、服务介绍、咨询流程、定制咨询、公司介绍、FAQ 和联系方式。
...
首页主要视觉已改为 `public/images/hero/` 中的 9 张 AI 主题图片轮播。轮播顺序、双语 alt 文案以及桌面端/手机端裁剪位置集中配置在 `data/siteConfig.ts` 的 `heroSlides` 中。

这些图片仅作为贵州旅游主题视觉素材使用，不应被宣传为某个具体贵州景点的真实实拍或黔林旅行社带团记录。公司介绍和定制区域图片仍在 `data/siteConfig.ts`，目的地图片在 `data/destinations.ts`，线路图片在 `data/tours.ts`。

上线前仍需确认所有图片的版权、授权和最终展示效果。
