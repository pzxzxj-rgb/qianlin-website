...
后台包含公司资料、联系方式、网站图片、旅行灵感 / 参考方案、目的地、咨询管理和 Theme Studio / 网站可视化编辑器。目的地管理不提供图片新增或修改，已有首页目的地图片继续保留。无图片目的地只能用于行程规划。后续如需更换目的地图片，应统一进入网站图片管理能力。

前台包含旅行偏好、目的地浏览、参考行程 Planner、咨询提交、移动咨询栏和主题系统。参考方案用于展示旅行方向，不包含在线预订、实时价格或实时库存。
...
迁移文件位于 `drizzle/`，当前最新迁移为 `0014_controlled_theme_studio.sql`。0011 会先检查历史同步任务是否跨租户，发现错配即中止，不会静默覆盖；之后用 `(tenant_id, inquiry_id)` 复合外键约束同租户关联。0012 扩展同步任务状态约束以支持 `dead_letter` 状态。0013 为咨询增加租户内唯一 `submission_id` 幂等键，并增加 Session 到期清理索引；历史咨询会获得迁移生成的兼容键，既有政策版本迁移为 `privacy-2026-08-04`。0014 为每个租户建立受控的 draft / published Theme Studio 主题记录，并限制模板、字体、按钮、卡片和区块样式取值。本任务不执行远程 D1 migration，不连接远程 D1，不部署生产环境。

上线前请逐项完成：[docs/prelaunch-checklist.md](docs/prelaunch-checklist.md)
